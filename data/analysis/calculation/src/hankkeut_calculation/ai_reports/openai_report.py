...
def _validate_report_against_input(
    ...
    for action in report["recommended_actions"]:
        if action["content_type"] not in allowed_types:
            raise ValueError("recommended action contains a content type not selected by the backend")
...
Recommended-actions policy overrides every earlier recommended-action instruction: create at least one autonomous and concrete idea for a selected_content_types item; use the current comparison data and approved web-source cases to suggest how existing tourism resources can be reused to address unmet demand; avoid discussing Kakao collection quality and avoid merely listing metric values; include content_type; and use respectful Korean formal speech.\n\nThe following interpretation rules are normative and override any intuitive but conflicting interpretation:\n\n""" + interpretation_rules
